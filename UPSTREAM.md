...
Phase 1이 2026-04-11에 완료되었다. opencode-nexus는 `@moreih29/nexus-core ^0.1.2`를 devDependency로 consume하는 Big-Bang Cutover 전환을 완료했다(`phase1-nexus-core-adoption` 브랜치, 4-commit bisectable sequence: `a9cb773` → `4df4451` → `ff813e6` → `ee52ed5`). claude-nexus는 Phase 2에서 동일 패키지를 consume하는 방향으로 전환한다. 두 프로젝트는 여전히 sibling 관계이며, `@moreih29/nexus-core`가 canonical source다.
...
### `.nexus/` 구조 — 부분 resolved (2026-04-12)
...
**도구 인프라**는 4-layer `.nexus/core/{identity,codebase,memory,reference}`를 코드 차원에서 유지한다. `nx_init` / `nx_sync` / `nx_core_*` / `nx_briefing` 같은 도구가 호출 시 `mkdir(recursive: true)`로 자동 재생성하며, 자동 생성물(`recent-cycle-summary.md`, `recent-changes.md`, `decision-log.md`)은 이 4-layer에 작성된다. 사람이 직접 들여다볼 일이 거의 없는 운영 인프라.

**원래 결정의 일부 번복**: 2026-04 plan session #1에서는 4-layer 전체 유지를 intentional divergence로 결정했으나, 사람 문서의 flat 구조가 표준에 부합하고 cognitive overhead를 줄인다는 판단으로 사람 문서만 flat으로 분리했다. 도구 인프라의 4-layer 폐지는 `nx_core_*` / `nx_briefing` redesign이 필요하므로 별도 plan session에서 다룬다 (참조: `.nexus/memory/phase1-followup.md`). bridge `docs/bridge/nexus-core-bootstrap.md` §9.4는 anti-patterns #1(bridge 재작성 금지)에 따라 갱신하지 않는다.
