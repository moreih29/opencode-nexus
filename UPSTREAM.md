...
### post_tool_use tool-log scope

nexus-core consumer-implementation-guide §9 post_tool_use는 `.nexus/state/tool-log.jsonl`에 모든 도구 사용을 로깅하는 것을 SHOULD로 명시하고, 이 관행이 "observed across all consumers"라고 기술한다. 그러나 schema definition은 pending 상태다.

opencode-nexus는 이 divergence를 consumer 재량 범위 내에서 다음과 같이 한정한다:

- **edit-like tool만 로깅** — `src/plugin/hooks.ts`의 `tool.execute.after` 훅에서 `isEditLikeTool(input.tool)` 분기 내에서만 `appendToolLogEntry`를 호출한다. 즉, 파일을 수정하는 도구(`edit`, `write` 등)만 tool-log.jsonl에 기록한다.
- **`.nexus/memory/` read는 별도 훅** — `.nexus/memory/` 디렉토리 read 감시는 `memory-access.jsonl` 별도 훅이 커버하며, `src/shared/memory-access.ts`의 `upsertMemoryAccessRecord`가 담당한다(nexus-outputs-contract 규정).

#### 근거 (Decision rationale)

1. **nexus-core "schema definition pending"** — 공식 문서에서 schema가 확정되지 않았으므로 consumer가 재량껏 범위를 한정할 수 있다.
2. **files_touched aggregation 완충** — 현재 opencode-nexus의 bounded-tier resume 시 `files_touched` 정보는 edit-like tool 로그만으로 충분히 복원 가능하다.
3. **memory read는 별도 contract 존재** — nexus-outputs-contract.md가 `.nexus/state/memory-access.jsonl`을 별도 규정하고 있어, 전면 tool-log와 중복 기록은 불필요하다.
4. **JSONL 팽창 비용 대비 효용** — 모든 도구( read, bash, grep 등)를 로깅하면 파일 크기가 급격히 증가하나, 현재 use case(bounded resume, change audit)에 필요한 정보는 edit-like subset으로 충분하다.

#### 재정렬 트리거 (Re-alignment trigger)

nexus-core가 tool-log.jsonl의 공식 schema를 확정하여 SHOULD 명세가 강화되면, 그 시점에 전면 로깅으로 전환한다.

#### 참조 링크

- `~/workspaces/areas/nexus-core/docs/consumer-implementation-guide.md` §9 post_tool_use — "SHOULD append a log entry... (This practice is observed across all consumers; nexus-core does not yet define a schema for this file — schema definition pending.)"
- `~/workspaces/areas/nexus-core/docs/nexus-outputs-contract.md` — `.nexus/state/memory-access.jsonl` 규정 섹션
