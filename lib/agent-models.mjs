...
async function selectCycleAction() {
  return select({
    ...
    options: [
      { value: "done", label: "마침", hint: "변경사항을 저장하고 종료" },
      { value: "continue", label: "계속 매핑", hint: "저장하지 않고 다른 에이전트 매핑 추가" },
      { value: "cancel", label: "취소", hint: "변경사항을 저장하지 않고 종료" },
    ],
    initialValue: "done",
  });
}
...
function formatModelHint(config, agentId) {
  const spec = getAgentModelSpec(config, agentId);
  return spec.value ? `현재: ${spec.value}` : "inherit";
}
  ...
  while (true) {
    ...
    if (isCancel(selectedAgents)) {
      ...
      const provider = await select({
        ...
        options: [...providersById.entries()].map(([providerId, models]) => ({
          value: providerId,
          label: providerId,
