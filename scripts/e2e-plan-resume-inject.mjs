 ...
 *
 *   orchestration.opencode.json fixture
...
// 8. full chain on on-disk orchestration.opencode.json fixture
{
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexus-resume-test-"));
  const filePath = path.join(tmpDir, "orchestration.opencode.json");
  const fixture = {
    schema_version: 1,
  ...
  try {
    ...
    if (out.task_id === "ses_FIXTURE") {
      p("full chain: orchestration.opencode.json → continuity → hints → inject");
    } else {
