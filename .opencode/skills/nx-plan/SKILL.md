...
When triggered with `[plan:auto]` or invoked via `skill({ name: "nx-plan" })`, run the full planning process **without user interaction**:
...
| Scenario | Approach |
|----------|----------|
| Codebase orientation | `task({ subagent_type: "explore", prompt: "<file/code search task>" })` for codebase exploration |
| External research needed | `task({ subagent_type: "researcher", prompt: "<research question>" })` for web search |
