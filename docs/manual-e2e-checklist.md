...
1. `meet` / `run`에서 오케스트레이션 코어 continuity가 잘 동작하는지
...
- meet projection: `.nexus/state/meet.opencode.json`
- task cycle 상태: `.nexus/state/tasks.json`
- 감사 로그: `.nexus/logs/opencode-audit/all.jsonl`
- 세션 로그: `.nexus/logs/opencode-audit/sessions/<session-id>/session.jsonl`
- 서브에이전트 로그: `.nexus/logs/opencode-audit/sessions/<session-id>/subagents/<invocation-id>.jsonl`
- 종료된 cycle 이력: `.nexus/history.json`

## 1. Meet continuity 검증

### 목적

`meet`에서 한 번 호출한 HOW 참여자에게, 이후 follow-up 질문을 했을 때 단순 요약이 아니라 실제 orchestration continuity를 기반으로 이어지는지 확인합니다.

### 절차

1. HOW 참여자 한 명으로 meet를 시작합니다.
...
- meet 관련 출력
  - `nxMeetResume` / `nxMeetFollowup` 성격의 출력에 `opencode_task_tool_resume_handle`가 보여야 함
...
- `tasks.json`
  ...
  - status 변경이 반영되어야 함
- task tool 출력
  - `nxTaskAdd`는 `nexus_task_id`를 반환해야 함
  - `nxTaskUpdate`는 `nexus_task_id`와 `status`를 반환해야 함
- edit guard
...
- Nexus task 출력은 `nexus_task_id`를 사용해야 함
- meet follow-up 출력은 `opencode_task_tool_resume_handle`을 노출해야 함
...
3. `meet.opencode.json`에서 panel membership과 summary를 본다
