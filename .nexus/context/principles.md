...
**Phase 2 선언 (v0.10.0+)**: opencode-nexus는 더 이상 독립적인 orchestration runtime 구현이 아니라, `@moreih29/nexus-core`의 **runtime substrate consumer**다. Canonical 로직은 nexus-core에서 관리되며, opencode-nexus는 OpenCode-specific harness adapter 역할만 수행한다.
...
**Thin-wrapper integrity (v0.10.0+)**: opencode-nexus는 nexus-core canonical 범위를 local에서 재구현하지 않는다. 모든 orchestration 로직은 nexus-core substrate에 위임하며, opencode-nexus는 harness-specific adapter 역할에 충실한다. 이 원칙은 19,755 LOC 삭제를 통해 실현되었다.
