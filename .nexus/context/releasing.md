...
### 5-3. MCP binary reachability (필수)

OpenCode는 `opencode.json`의 `mcp.nx.command`를 spawn하는 방식으로 MCP 서버를 구동한다. `command` 배열의 첫 인자가 **사용자 환경의 `$PATH`에서 실행 가능해야** 서버가 기동된다. 실패하면 OpenCode는 silently 해당 서버 없이 계속 동작하고 `nx_*` 툴은 모두 unavailable이 된다. 이 상태는 스펙 위반이 아니라 설치 경로 누락이므로 정적 검증만으로는 잡히지 않는다.

격리된 임시 prefix로 글로벌 설치를 시뮬레이션해 확인한다:
...
timeout 2 "$NPM_CONFIG_PREFIX/bin/nexus-mcp" </dev/null
```

반드시 확인할 항목:

- `$NPM_CONFIG_PREFIX/bin/` 아래에 `opencode-nexus`와 `nexus-mcp` 심볼릭 링크가 **모두** 생성되는지 확인
- `opencode.json`의 `mcp.nx.command`에 쓰이는 모든 실행 파일명이 위 bin 디렉토리에 실제로 존재하는지 확인
- `nexus-mcp`를 직접 spawn했을 때 Node/Bun 런타임 해석 에러 없이 stdio 대기 상태로 진입하는지 확인 (timeout으로 종료해도 exit 0 / signal termination)
- OpenCode가 설치된 환경이라면 `opencode mcp list`에서 `nx` 항목이 `Executable not found in $PATH` 없이 떠야 한다

이 단계를 생략하면 사용자 환경에 따라 MCP가 조용히 기동되지 않는 회귀(v0.13.1~v0.13.3에서 발생)가 반복된다. 의존성이 제공하는 바이너리를 wrapper의 `command`에 그대로 쓰는 경우, 그 바이너리를 wrapper의 `package.json` `bin` 섹션에도 re-export 선언하는 것이 원칙이다. 그렇지 않으면 transitive dep의 bin은 글로벌 PATH에 symlink되지 않는다.
