...
- `.nexus/rules/` — 강제 컨벤션
- `.nexus/state/` — runtime 데이터 (사람이 직접 작성/관리하는 영역. 자동 생성물 cache는 plan #23에서 폐지). nexus-core v0.4.0의 `rule:harness-state-namespace`에 따라 opencode-nexus 고유 파일(`plan.extension.json`)은 `.nexus/state/opencode-nexus/` 네임스페이스 디렉토리로 격리된다. `plan.extension.json`은 공통 `plan.json`의 extension이며 스키마는 리포지토리 루트 `state-schemas/`(`plan.extension.schema.json`, draft 2020-12)에서 선언한다.
