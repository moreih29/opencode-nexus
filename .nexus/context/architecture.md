...
- `core.ts`: 호출 등록(`registerStart`/`registerEnd`), `.nexus/state/orchestration.opencode.json` 기반 상태 영속화.
...
#### 자동 생성물 (auto-generated)

`nx_init` / `nx_sync` / `nx_task_close` 도구가 사이클 완료 또는 동기화 시점에 `.nexus/state/auto/`에 자동 생성물을 기록한다.

| 파일 | 생성 시점 | 내용 |
|------|-----------|------|
| `recent-cycle-summary.md` | `nx_sync` | 최근 사이클 요약 |
| `recent-changes.md` | `nx_sync` | 최근 변경 내역 |
| `decision-log.md` | `nx_sync` | 의사결정 로그 |
| `roadmap-sync.md` | `nx_sync` | 로드맵 동기화 스냅샷 |
| `cycle-{N}.md` | `nx_task_close` | 사이클 N 메모리 노트 |

사람 문서(`.nexus/context/`, `.nexus/memory/`, `.nexus/rules/`)와 자동 생성물(`.nexus/state/auto/`)은 디렉토리가 분리되어 충돌이 없다.
