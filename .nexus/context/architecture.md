...
- \`hooks.ts\`: 7개 훅 — \`event\`(세션 초기화), \`tool.execute.before\`(가드레일·KNOWLEDGE_INDEX 주입), \`tool.execute.after\`(완료 처리·knowledge-index 캐시 무효화), \`chat.message\`(프롬프트 저장), \`command.execute.before\`(종료 경고), \`experimental.chat.system.transform\`(모드 감지·시스템 프롬프트 주입), \`experimental.session.compacting\`(multi-line 상태 스냅샷 주입).
  - \`tool.execute.before\`: 편집류 도구(edit-like tools)에 대해 **태스크 사이클 상태에 따른 차등 가드레일**을 적용한다. \`idle\` 모드(태스크 없음) 또는 \`free\` 편집 모드에서는 차단하지 않으며, \`completed-open\` 상태에서만 보호(block/warning)가 작동한다. 추가로 \`task\` 도구 호출 시에는 \`appendKnowledgeIndexToTaskArgs\`를 통해 **KNOWLEDGE_INDEX 블록을 args.prompt/description에 주입**하여 subagent_spawn continuity를 지원한다.
  - \`tool.execute.after\`: 도구 실행 완료 후 **미완료 소유자 태스크 경고**를 추가한다. 실행된 도구의 소유자(owner)와 일치하는 태스크가 \`pending\` 또는 \`in_progress\` 상태로 남아 있으면, 해당 태스크 목록을 경고 메시지에 포함시킨다. 또한 편집 대상 경로가 \`.nexus/{context,memory,rules}\` 하위일 경우 \`invalidateKnowledgeIndex(projectRoot)\`를 호출하여 **knowledge-index 캐시를 무효화**한다.
  ...
  - \`experimental.session.compacting\`: 세션 컴팩팅 시 **multi-line 구조의 상태 스냅샷**을 생성한다. \`[nexus-state-snapshot]\` prefix와 함께 \`active mode\`, \`plan\`(topic 및 issues 제목+status, 최대 20개), \`tasks\`(제목+status 및 ready-task set, 최대 30개), \`knowledge_index\`(context/memory/rules 파일명 각 최대 50개), \`active agents\`(coordination_label 포함) 섹션을 포함한다.
...
- \`state.ts\`: \`.nexus/\` 디렉터리 초기화(비파괴), 태스크 요약. **session_start 방어** 동작으로 \`.nexus/.gitignore\` 자동 작성 및 루트 legacy \`agent-tracker.json\` silent removal을 수행한다.
...
- \`knowledge-index.ts\`: \`.nexus/{context,memory,rules}\`의 마크다운 파일 목록을 수집·캐싱. \`collectKnowledgeIndex\`, \`formatKnowledgeIndexBlock\`, \`invalidateKnowledgeIndex\`, \`appendKnowledgeIndexToTaskArgs\` 제공. 모듈 레벨 Map<projectRoot, {snapshot, collectedAt}> 캐시 사용.
- \`context-snapshot.ts\`: \`experimental.session.compacting\` 훅용 multi-line 상태 스냅샷 생성기. \`buildCompactionStateSnapshot(paths)\` — mode/plan/tasks/knowledge_index/active agents 섹션 포함.
...
- **Dependency**: \`@moreih29/nexus-core ^0.12.0\` (devDependency 전용 — 최종 사용자 환경에 미설치)
