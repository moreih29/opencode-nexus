...
## §9.4 — postinstall은 mutation authority가 아니다 (v0.11.0 추가)

**Incorrect pattern**:
```javascript
// scripts/postinstall.mjs
copyFileSync(skillSrc, consumerSkillDst);
mergeOpencodeJson(consumerConfigPath, managedKeys);
```

**Correct pattern**:
```javascript
// scripts/postinstall.mjs — minimal hint only
console.log("Run: bunx opencode-nexus install --scope=project");
// zero filesystem mutation
```

**근거**:
- npm/bun/pnpm/yarn 각자의 lifecycle script 정책이 다름:
  - npm: `postinstall` 기본 실행
  - bun 1.3+: trust 없이 차단
  - pnpm v10: dependency lifecycle 제한
  - yarn berry: `enableScripts: false` 기본
- postinstall에 설치 책임을 두면 "어떤 사용자는 동작, 어떤 사용자는 안 됨"의 비결정성 발생 (plan #58 Issue #3 architect analysis)
- CLI가 canonical mutator여야 PM-independent UX 보장
- npm uninstall lifecycle 부재로 postinstall-created artifacts cleanup 자동화 불가 → CLI uninstall 필수

**Enforcement**:
- scripts/postinstall.mjs는 `console.log`만 호출
- filesystem write API (copyFileSync, writeFileSync, mkdirSync, renameSync 등) import 금지
- self-install guard 유지
...
const fragment = JSON.parse(readFileSync(
  "node_modules/@moreih29/nexus-core/opencode.json.fragment"
));
mergeIntoConsumerConfig(fragment);
```

**Correct pattern**:
```javascript
// lib/install-spec.mjs — wrapper-local canonical constants
export const PACKAGE_NAME = "opencode-nexus";
export const MCP_SERVER_CONFIG = { type: "local", command: ["nexus-mcp"] };
export const DEFAULT_AGENT = "lead";
// ...

// CLI consumes these directly
import { PACKAGE_NAME, MCP_SERVER_CONFIG } from "./install-spec.mjs";
```

**근거**:
- `opencode.json.fragment`는 (a) nexus-core tarball의 `files` 필드에 포함되지 않아 consumer에게 배포되지 않음, (b) 실제 OpenCode canonical schema와 불일치(upstream nexus-core#43 참조), (c) fragment 스키마가 runtime plugin auto-registration 이후 중복됨
- CLI가 fragment를 입력으로 삼으면 nexus-core의 낮부 자산 드리프트에 종속됨
- wrapper-local hardcoded spec은 "얇은 경계"로서 향후 nexus-core가 공식 `canonical-install-spec` export를 제공할 때 import로 교체 가능 (option I → option II migration path)

**Enforcement**:
- lib/install-spec.mjs에만 canonical constants 선언
- CLI/lib 어느 파일도 `opencode.json.fragment` 문자열 참조 금지 (grep 검증)
- 향후 nexus-core 공식 export 제공 시 install-spec을 re-export 어댑터로 전환
