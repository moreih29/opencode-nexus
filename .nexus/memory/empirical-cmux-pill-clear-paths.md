...
- v0.16.1 (2026-04-23) — (1) `Needs Input` pill 아이콘을 `bell` → `bell.fill`(SF Symbols)로 교체. 다른 cmux 플러그인(`claude_code`)과 시각 일관성. (2) **`OPENCODE_NEXUS_DEBUG_PREVIEW=1|true` opt-in 진단 환경변수 도입**. v0.16.0 preview가 fallback으로 빠지는 원인을 payload 단위로 확정하기 위한 임시 도구 — `message.part.updated` 이벤트마다 `part.type` / `sessionID` / `rootKnown` / `textLen`을 cmux log에 기록. flood 방지 위해 opt-in. v0.16.2에 정식 fix 완료 후 제거 또는 유지 판단 예정.
- v0.16.2 (2026-04-23) — **preview cache 경로 정식 fix**. 원인: v0.16.0에서 `session.status busy`를 턴 경계로 간주해 `rootSessionLastText.delete`를 호출했는데, OpenCode는 한 턴에 busy 이벤트를 여러 번(관측 4회) 발행한다. 마지막 busy가 step-finish 이후에 fire되어 방금 캐싱된 assistant text를 지워버려 session.idle에서 fallback `"Response ready"`가 귀결. Fix: (a) `session.status busy` 분기에서 cache delete 제거. (b) `assistantTurnActive: Map<string, boolean>` 상태 추가 — `message.part.updated`의 `step-start` part에서 true로 set하고 캐시 초기화, session.idle/deleted에서 delete. (c) text part는 `assistantTurnActive === true`일 때만 캐싱되므로 user input text나 기타 pre-assistant 페이로드가 preview로 누출되지 않음. v0.16.1의 `OPENCODE_NEXUS_DEBUG_PREVIEW` 진단 로그는 유지(유사 이슈 재현 시 재사용). e2e cmux-l/m/n 수정, cmux-p 의미 교체(step-start 기반 reset), cmux-q 신규(step-start 없는 text는 캐시 안 됨) 추가.
- 릴리즈 체크리스트 §5-5-1 보강 — 이후 릴리즈부터는 두 clear 경로 호출 + serialize 순서 보장 + `bell.fill` 아이콘 + step-start 기반 assistant 턴 경계가 release gate 기준.
- v0.16.3 (2026-04-23) — MessageAbortedError를 session.error에서 분기해 Needs Input 전환 경로로 흡수. 사용자 인터럽트가 에러 UX로 노출되지 않도록 정리. e2e cmux-r/s 신규, cmux-h는 역할 이관으로 제거.
- v0.16.4 (2026-04-24) — **로그 수명 축 대응(이 파일의 pill 축과 별개 범주)**. idle→running 전이 시 `cmux clear-log`를 선행 호출하는 `sessionRunning: Set<string>` state machine 추가. append-only cmux 로그 패널에 stuck된 과거 에러 엔트리(MessageAbortedError 등)가 새 사용자 턴 시작과 함께 자동 제거되도록 한다. busy 4×/턴 특성은 이 파일 경로 2와 같은 맥락으로 Set 기반 dedup으로 흡수. 세부 근거와 anti-patterns는 `empirical-cmux-log-turn-reset.md` 참조. e2e cmux-t/u/v/w 4개 시나리오 신규.
